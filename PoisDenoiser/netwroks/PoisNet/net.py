...
class PoisNet(nn.Module): # this class is almost fully based on 
                          # UDNet class from pydl.nnLayers.modules
    ...
    def __init__(self, kernel_size=(7,7),\
        ...
        self.resRBFPois = nn.ModuleList([modules.ResRBFPoisLayer(kernel_size,\
                                      ...
                                      normalizedWeights,\
                                      zeroMeanWeights,\
                                      prox_param) for i in range(self.stages)])      

        self.bbProj = nn.ReLU() #nn.Hardtanh(min_val=clb, max_val=cub)
        
    def forward(self, input, noisy):

        for m in self.resRBFPois:
            input = m(input, noisy, self.rbf_data_lut.type_as(input))
